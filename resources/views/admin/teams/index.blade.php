@extends('layouts.admin')

@section('title', 'Tim')

@section('breadcrumb')
    <a href="{{ route('admin.dashboard') }}" class="hover:text-indigo-600 transition-colors">Admin</a>
    <i class="fa-solid fa-chevron-right text-[10px]"></i>
    <span class="text-neutral-900 font-semibold">Tim</span>
@endsection

@section('content')
    <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden flex flex-col">
        <div class="px-6 py-5 border-b border-zinc-100 flex flex-col sm:flex-row gap-4 justify-between bg-neutral-50/50">

            <!-- Filter & Search Form -->
            <form action="{{ route('admin.teams.index') }}" method="GET"
                class="flex flex-col sm:flex-row flex-wrap gap-3 w-full">
                <!-- Search -->
                <div class="relative w-full sm:max-w-xs flex-shrink-0">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <i class="fa-solid fa-magnifying-glass text-gray-400 text-sm"></i>
                    </div>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama tim..."
                        class="w-full bg-white text-neutral-900 rounded-xl py-2.5 pl-9 pr-4 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 border border-zinc-200 transition-all placeholder:text-gray-400">
                </div>

                <!-- Status -->
                <select name="status"
                    class="bg-white text-neutral-900 rounded-xl py-2.5 px-4 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 border border-zinc-200 transition-all sm:w-[160px]"
                    onchange="this.form.submit()">
                    <option value="">Semua Status</option>
                    <option value="published" {{ request('status') === 'published' ? 'selected' : '' }}>Tampil (Published)
                    </option>
                    <option value="unpublished" {{ request('status') === 'unpublished' ? 'selected' : '' }}>Disembunyikan
                    </option>
                </select>

                @if (request()->hasAny(['search', 'status']))
                    <a href="{{ route('admin.teams.index') }}"
                        class="btn btn-sm text-red-500 hover:bg-red-50 hover:text-red-600 border-transparent shadow-none px-3 flex-shrink-0">
                        Reset
                    </a>
                @endif

                <div class="sm:ml-auto">
                    <a href="{{ route('admin.teams.create') }}" class="btn btn-primary btn-sm h-full">
                        <i class="fa-solid fa-plus mt-0.5"></i> Tambah Anggota
                    </a>
                </div>
            </form>
        </div>

        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-white text-xs font-bold text-gray-500 uppercase tracking-wider">
                        <th class="px-6 py-4 border-b border-zinc-100">Anggota</th>
                        <th class="px-6 py-4 border-b border-zinc-100">Jabatan</th>
                        <th class="px-6 py-4 border-b border-zinc-100 text-center">Status</th>
                        <th class="px-6 py-4 border-b border-zinc-100 text-right">Aksi</th>
                    </tr>
                </thead>
                <tbody class="text-sm font-medium text-neutral-800 divide-y divide-zinc-100">
                    @forelse($teams as $team)
                        <tr class="hover:bg-neutral-50/50 transition">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-10 h-10 rounded-full border border-zinc-200 overflow-hidden bg-neutral-100 shrink-0">
                                        @if ($team->photo)
                                            <img src="{{ $team->image_url }}" alt="{{ $team->name }}"
                                                class="w-full h-full object-cover">
                                        @else
                                            <div
                                                class="w-full h-full flex items-center justify-center text-gray-400 text-sm font-bold">
                                                {{ substr($team->name, 0, 1) }}
                                            </div>
                                        @endif
                                    </div>
                                    <div>
                                        <p class="font-bold text-neutral-900 leading-tight">{{ $team->name }}</p>
                                    </div>
                                </div>
                            </td>

                            <td class="px-6 py-4">
                                <p class="text-gray-600">{{ $team->role }}</p>
                            </td>

                            <td class="px-6 py-4 text-center">
                                @php
                                    $statusColor = match ($team->status) {
                                        'published' => 'bg-green-100 text-green-700',
                                        'unpublished' => 'bg-gray-100 text-gray-700',
                                        default => 'bg-gray-100 text-gray-700',
                                    };
                                    $statusLabel = match ($team->status) {
                                        'published' => 'Tampil',
                                        'unpublished' => 'Sembunyi',
                                        default => ucfirst($team->status),
                                    };
                                @endphp
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider {{ $statusColor }}">
                                    {{ $statusLabel }}
                                </span>
                            </td>

                            <td class="px-6 py-4 text-right whitespace-nowrap">
                                <form action="{{ route('admin.teams.toggle-status', $team->id) }}" method="POST"
                                    class="inline-block mr-1">
                                    @csrf
                                    @method('PATCH')
                                    @if ($team->status === 'published')
                                        <button type="submit"
                                            class="w-8 h-8 rounded-lg bg-amber-50 text-amber-600 hover:bg-amber-500 hover:text-white flex items-center justify-center transition-colors"
                                            title="Sembunyikan">
                                            <i class="fa-solid fa-eye-slash"></i>
                                        </button>
                                    @else
                                        <button type="submit"
                                            class="w-8 h-8 rounded-lg bg-green-50 text-green-600 hover:bg-green-500 hover:text-white flex items-center justify-center transition-colors"
                                            title="Tampilkan">
                                            <i class="fa-solid fa-eye"></i>
                                        </button>
                                    @endif
                                </form>

                                <a href="{{ route('admin.teams.edit', $team->id) }}"
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-500 hover:text-white transition-colors"
                                    title="Edit">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>

                                <form action="{{ route('admin.teams.destroy', $team->id) }}" method="POST"
                                    class="inline-block ml-1 delete-form"
                                    data-confirm="Apakah Anda yakin ingin menghapus anggota tim ini?">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="w-8 h-8 rounded-lg bg-red-50 text-red-600 hover:bg-red-500 hover:text-white flex items-center justify-center transition-colors"
                                        title="Hapus">
                                        <i class="fa-solid fa-trash-can"></i>
                                    </button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-gray-500">
                                <div
                                    class="w-16 h-16 mx-auto bg-gray-50 rounded-full flex items-center justify-center text-gray-400 mb-3 text-2xl">
                                    <i class="fa-solid fa-users"></i>
                                </div>
                                <p class="font-bold text-neutral-900 mb-1">Belum ada anggota tim</p>
                                <p class="text-sm">Tambahkan anggota tim untuk ditampilkan di halaman Tentang Kami.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>

        @if ($teams->hasPages())
            <div class="px-6 py-4 border-t border-zinc-100 bg-neutral-50/30">
                {{ $teams->links() }}
            </div>
        @endif
    </div>
@endsection

...
@section('breadcrumb')
    ...
    <i class="fa-solid fa-chevron-right text-[10px]"></i>
    <span class="text-neutral-900 font-semibold">{{ $team->name }}</span>
@endsection
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <select id="status" name="status" required
                                ...
                                <option value="published"
                                    {{ old('status', $team->status) == 'published' ? 'selected' : '' }}>Published (Tampil)
                                </option>
                                <option value="unpublished"
                                    {{ old('status', $team->status) == 'unpublished' ? 'selected' : '' }}>Unpublished
                                    (Sembunyi)</option>
                            </select>
                            @error('status')
                                <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            @if ($team->photo)
                                <div
                                    class="mb-3 w-20 h-20 rounded-full border border-zinc-200 overflow-hidden bg-neutral-50">
                                    <img src="{{ $team->image_url }}" alt="Avatar" class="w-full h-full object-cover">
                                </div>
                            @endif
                            ...
                            <p class="text-[11px] text-gray-500 mt-1.5">Kosongkan jika tidak ingin mengubah foto saat ini.
                            </p>
                            @error('photo')
                                <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
                            @enderror

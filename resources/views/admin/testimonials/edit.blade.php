...
@section('breadcrumb')
  ...
  <i class="fa-solid fa-chevron-right text-[10px]"></i>
  <span class="text-neutral-900 font-semibold">{{ $testimonial->name }}</span>
@endsection
      ...
      <div class="space-y-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
           ...
           <div>
             ...
             <input type="text" id="role" name="role" value="{{ old('role', $testimonial->role) }}" placeholder="Mis. Pegawai Kantoran" 
               class="w-full bg-neutral-50 rounded-xl py-3 px-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500 border border-zinc-200">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
           ...
           <div>
             ...
             <select id="status" name="status" required class="w-full bg-neutral-50 rounded-xl py-3 px-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500 border border-zinc-200">
               <option value="published" {{ old('status', $testimonial->status) == 'published' ? 'selected' : '' }}>Published (Tampil)</option>
               <option value="pending" {{ old('status', $testimonial->status) == 'pending' ? 'selected' : '' }}>Pending (Menunggu)</option>
               <option value="unpublished" {{ old('status', $testimonial->status) == 'unpublished' ? 'selected' : '' }}>Unpublished (Sembunyi)</option>
             </select>
        ...
        <div>
          ...
          <textarea id="message" name="message" rows="4" required class="w-full bg-neutral-50 rounded-xl py-3 px-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500 border border-zinc-200">{{ old('message', $testimonial->message) }}</textarea>
          @error('message') <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p> @enderror
        </div>

        <!-- Foto -->
        <div>
          <label class="block text-sm font-bold text-neutral-900 mb-2">Foto / Avatar</label>
          @if($testimonial->photo)
            <div class="mb-3 w-20 h-20 rounded-full border border-zinc-200 overflow-hidden bg-neutral-50">
               <img src="{{ $testimonial->image_url }}" alt="Avatar" class="w-full h-full object-cover">
            </div>
          @endif
          ...
          <p class="text-[11px] text-gray-500 mt-1.5">Kosongkan jika tidak ingin mengubah foto saat ini.</p>
